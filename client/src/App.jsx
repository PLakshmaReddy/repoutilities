...
const RepoDetail = ({ repo }) => {
		...
		const fetchBranches = async () => {
			try {
				const response = await axios.get(
					`${API_BASE}/repos/${repo.id}/branches`,
				);
	...
	return (
		<div className="bg-white shadow rounded-lg overflow-hidden">
			...
			<div className="p-6">
				{activeTab === "compare" && (
					<BranchCompare repoId={repo.id} branches={branches} />
				)}
				{activeTab === "release" && (
					<ReleaseCreator repoId={repo.id} branches={branches} />
				)}
				{activeTab === "reports" && (
					<ReportsView repoId={repo.id} branches={branches} />
				)}
				{activeTab === "deployments" && (
					<DeploymentsView repoId={repo.id} branches={branches} />
				)}
			</div>
		</div>
	);
				...
				<div className="space-y-4">
					...
					{diffData.diffs.map((diff, idx) => (
						<div key={idx} className="border rounded-md overflow-hidden">
							<div className="bg-gray-50 px-4 py-2 border-b font-mono text-sm">
								{diff.file}
							</div>
...
const ReportsView = ({ repoId, branches }) => {
	...
	const fetchReports = useCallback(async () => {
		setLoading(true);
		try {
			const response = await axios.get(`${API_BASE}/repos/${repoId}/reports`, {
				params: { branch },
			});
			...
			{loading ? (
				...
				reports && (
					<div className="grid grid-cols-1 md:grid-cols-3 gap-6">
						<ReportCard
							title="Code Coverage"
							status={reports.coverage.status}
							data={[
						...
						<ReportCard
							...
							data={[
								{
									label: "High",
									value: reports.security.high,
									color: "text-red-600",
								},
								{
									label: "Medium",
									value: reports.security.medium,
									color: "text-yellow-600",
								},
								{
									label: "Low",
									value: reports.security.low,
									color: "text-blue-600",
								},
							]}
						/>
		...
		<div className="space-y-2">
			{data.map((item, idx) => (
				<div key={idx} className="flex justify-between text-sm">
					...
					<span className={`font-bold ${item.color || "text-gray-900"}`}>
						{item.value}
					</span>
				</div>
			))}
...
const DeploymentsView = ({ repoId, branches }) => {
	...
	const fetchDeployments = useCallback(async () => {
		...
		try {
			const response = await axios.get(
				`${API_BASE}/repos/${repoId}/deployments`,
				{ params: { branch } },
			);
	...
	useEffect(() => {
		if (branch) fetchDeployments();
	}, [branch, fetchDeployments]);
